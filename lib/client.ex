defmodule Proj4.Client do
  ...
  def init([client_ID, num_of_msgs, zipf_no_of_subs, disconnected_client]) do
    ...
    receive do
      {:registerConfirmation} -> IO.puts("@Client_#{client_ID} registration Successful")
    end
  ...
  def split([head | tail], l) do
    unless Node.alive?() do
      try do
        ...
        _ -> if l > 1, do: split(tail, l - 1), else: IO.puts(IO.puts("Can not split further!"))
      end
    end
  end
  ...
  def handle_feed(client_ID) do
    receive do
      {:live, tweet_string} -> IO.inspect(tweet_string, label: "@Client_#{client_ID} Feed :")
    end
  ...
  def handle_all_tweets(client_ID) do
    ...
    receive do
      {:handle_my_tweets_atom, list} -> IO.inspect(list, label: "Client #{client_ID} all Tweets :")
    end
  end
  ...
  def handle_subscribed_to(client_ID) do
    ...
    receive do
      {:handle_subscribed_atom, list} ->
        if list != [], do: IO.inspect(list, label: "@Client_#{client_ID} Tweets Subscribed :")
    end
  end
  ...
  def handle_hashtag(hashtag, client_ID) do
    ...
    receive do
      {:handle_hashtag_atom, list} ->
        if list != [],
          do: IO.inspect(list, label: "@Client_#{client_ID} Tweets containing #{hashtag} :")
    end
  end
  ...
  def handle_mention(client_ID) do
    ...
    receive do
      {:handle_mention_atom, list} ->
        if list != [],
          do: IO.inspect(list, label: "@Client_#{client_ID} Tweets mentioning @#{client_ID} :")
    end
  end
end
